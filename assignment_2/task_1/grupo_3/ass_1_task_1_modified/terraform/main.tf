provider "aws" {
  region = var.region
}

resource "aws_vpc" "main" {
  cidr_block           = "10.0.0.0/16"
  enable_dns_support   = true
  enable_dns_hostnames = true

  tags = {
    Name = "classicmodels-vpc"
  }
}

resource "aws_internet_gateway" "igw" {
  count  = var.publicly_accessible ? 1 : 0
  vpc_id = aws_vpc.main.id

  tags = {
    Name = "classicmodels-igw"
  }
}

resource "aws_route_table" "public" {
  count  = var.publicly_accessible ? 1 : 0
  vpc_id = aws_vpc.main.id

  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.igw[0].id
  }

  tags = {
    Name = "classicmodels-public-rt"
  }
}

resource "aws_subnet" "subnet_a" {
  vpc_id                  = aws_vpc.main.id
  cidr_block              = "10.0.1.0/24"
  availability_zone       = "${var.region}a"
  map_public_ip_on_launch = false

  tags = {
    Name = "classicmodels-private-subnet-a"
  }
}

resource "aws_subnet" "subnet_b" {
  vpc_id                  = aws_vpc.main.id
  cidr_block              = "10.0.2.0/24"
  availability_zone       = "${var.region}b"
  map_public_ip_on_launch = false

  tags = {
    Name = "classicmodels-private-subnet-b"
  }
}

resource "aws_route_table_association" "subnet_a_public" {
  count          = var.publicly_accessible ? 1 : 0
  subnet_id      = aws_subnet.subnet_a.id
  route_table_id = aws_route_table.public[0].id
}

resource "aws_route_table_association" "subnet_b_public" {
  count          = var.publicly_accessible ? 1 : 0
  subnet_id      = aws_subnet.subnet_b.id
  route_table_id = aws_route_table.public[0].id
}

resource "aws_db_subnet_group" "default" {
  name       = "rds-subnet-group"
  subnet_ids = [aws_subnet.subnet_a.id, aws_subnet.subnet_b.id]

  tags = {
    Name = "classicmodels-db-subnet-group"
  }
}

resource "aws_db_parameter_group" "mysql_secure" {
  name   = "classicmodels-mysql-secure"
  family = "mysql8.0"

  parameter {
    name  = "require_secure_transport"
    value = "ON"
  }

  tags = {
    Name = "classicmodels-mysql-secure"
  }
}

resource "aws_security_group" "rds_sg" {
  name   = "classicmodels-rds-sg"
  vpc_id = aws_vpc.main.id

  dynamic "ingress" {
    for_each = var.allowed_cidr == null ? [] : [var.allowed_cidr]
    content {
      description = "Restricted MySQL access for lab client"
      from_port   = 3306
      to_port     = 3306
      protocol    = "tcp"
      cidr_blocks = [ingress.value]
    }
  }

  tags = {
    Name = "classicmodels-rds-sg"
  }
}

resource "aws_db_instance" "mysql" {
  identifier        = var.db_instance_identifier
  engine            = "mysql"
  engine_version    = "8.0"
  instance_class    = var.instance_class
  allocated_storage = 20
  storage_type      = "gp2"
  storage_encrypted = true

  db_name  = var.db_name
  username = var.db_username
  password = var.db_password

  db_subnet_group_name   = aws_db_subnet_group.default.name
  parameter_group_name   = aws_db_parameter_group.mysql_secure.name
  vpc_security_group_ids = [aws_security_group.rds_sg.id]
  publicly_accessible    = var.publicly_accessible

  auto_minor_version_upgrade = true
  backup_retention_period    = 7
  copy_tags_to_snapshot      = true
  deletion_protection        = var.deletion_protection
  skip_final_snapshot        = false
  final_snapshot_identifier  = "${var.db_instance_identifier}-final-snapshot"

  lifecycle {
    precondition {
      condition     = !var.publicly_accessible || var.allowed_cidr != null
      error_message = "publicly_accessible=true requires allowed_cidr with a trusted /32."
    }
  }

  tags = {
    Name = "classicmodels-db"
  }
}
